...
describe("select component Sandbox version", () => {
  ...
  it("should render select options with 'Placeholder test' as fisrt value", () => {
    ...
    const { getAllByTestId, getByText } = render(
      <Sandbox {...Sandbox.args} placeholder={placeHolderTest} choices={choices} name={"test-sandbox"} />
    );
    const selectOptions = getAllByTestId("select-option").map((label) => label.textContent);
